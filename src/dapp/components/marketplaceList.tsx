...
import TruthBoxCard from "./truthBoxCard";
import { testBox2List } from "../store/testBox";
...
export default function MarketplaceList({ page = 1, pageSize = 24 }: Props) {
    // 使用专业的渐进式显示Hook
    const {
        items,
        startReveal,
    ...
    return (
            <div className="w-full">
                ...
                >
                    {items.map((item) => (
                        <ProgressiveRevealCard
                            key={`marketplace-card-${item.index}`}
                            item={item}
                ...
                {/* 空状态显示 - 只在确认无数据时显示 */}
                {!isRevealing && revealedCount === 0 && testBox2List.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-12 text-center">
                        ...
                        <button
                            onClick={() => {
                                reset();
                                // 重新加载数据的逻辑
                                setTimeout(() => startReveal(testBox2List), 100);
                            }}
