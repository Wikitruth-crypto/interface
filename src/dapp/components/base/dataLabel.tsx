...
interface Props {
    data: DataType;
    variant?: "default" | "outline" | "card"; // 默认是default，outline是带边框，card是带背景的
    size?: "sm" | "default" | "lg";
    className?: string;
    /** 最小宽度，默认自动 */
    minWidth?: string;
...
export default function DataLabel({
    ...
    return (
        ...
        >
            <div style={{ fontSize: sizeStyles.titleFontSize }} className="text-muted-foreground font-mono mb-1">
                {data.label}
            </div>
            <Statistic
                value={data.value}
