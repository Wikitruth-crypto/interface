...
interface ISiweAuth{
    ...
    // ========== ==========
    // function setAddress() external; // 不需要在前端调用
    ...
    // ========== 验证函数 ==========
    // function testStatementVerification(bytes memory token, string calldata expectedStatement) external view returns (bool);
    
    // function testHasResourceAccess(bytes memory token, string calldata resource) external view returns (bool);
}
