...
export const useReadPrivateData_TruthBox = (
    ...
    // 从 secretStore 获取方法
    const getPrivateKey_TruthBox = useSecretStore(
        (state) => state.getPrivateKey_TruthBox
    );
    const setPrivateKey_TruthBox = useSecretStore(
        (state) => state.setPrivateKey_TruthBox
    );
    const getSiweSession = useSecretStore((state) => state.getSiweSession);
    
    // 订阅 secretStore 中的 SIWE session 变化（用于自动检测 token）
    // 注意：这里直接访问 secrets 状态，而不是调用方法，以便能够响应式更新
    const siweTokenFromStore = useSecretStore((state) => {
        if (!address || currentChainId === null || currentChainId === undefined) {
            return null;
        }
        const normalizedAddress = address.toLowerCase();
        const session = state.secrets[currentChainId]?.[normalizedAddress]?.siwe;
